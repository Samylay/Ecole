...
## Essential files
- @ROADMAP.md — executor contract, phased task queue, NEEDS-USER gates.
...
- No raw hex in components — design tokens only. No new dependency without Samy (NEEDS-USER task).
...
- Phase gates (backend, video strategy, curriculum, hosting) are Samy-only; queue questions as NEEDS-USER, never pick for him.
