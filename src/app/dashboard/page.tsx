...
export default function DashboardPage() {
  ...
  // Calculate time spent (mock: ~15 min per lesson)
  ...
  const labels = {
    fr: {
      ...
      noActivity: "Commencez à apprendre pour voir votre progression ici !",
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">{l.dashboard}</h1>
          {user.isLoggedIn && (
            <p className="text-gray-500 mt-1">{l.welcome}, {user.name}</p>
          )}
        </div>
        ...
        {enrolled.length === 0 ? (
          <div className="text-center py-20 animate-fade-in">
            ...
            <p className="text-gray-500 text-lg mb-4">{l.noActivity}</p>
            <Link href="/" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-indigo-700 transition-colors">
              {l.exploreCourses}
            </Link>
          </div>
        ) : (
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">{l.overallProgress}</h2>
            ...
            {/* Course Progress */}
            <h2 className="text-lg font-semibold text-gray-900 mb-4">{l.continueLearning}</h2>
              ...
              {enrolled.map((course) => {
                ...
                return (
                  <Link
                    ...
                    className="block bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-all hover:-translate-y-0.5 group"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500">{done}/{allLsns.length} {locale === "ar" ? "حصص" : locale === "fr" ? "leçons" : "lessons"}</p>
                        <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                          <div className={`${colors.accent} h-2 rounded-full progress-bar`} style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                      <span className="text-lg font-bold text-gray-400 group-hover:text-indigo-600 transition-colors">
                        {pct}%
                      </span>
                    </div>
                  </Link>
